    ...
    steps:
      ...
      - name: Determine Project Directory & Ensure Gradle Wrapper
        ...
        run: |
          ...
          mkdir -p "$RES_DIR/drawable" "$RES_DIR/mipmap" "$RES_DIR/mipmap-anydpi-v26"
...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <foreground android:drawable="@drawable/ic_launcher_foreground" />
          ...
          for MIPMAP_TARGET in "$RES_DIR/mipmap-anydpi-v26/ic_launcher.xml" "$RES_DIR/mipmap-anydpi-v26/ic_launcher_round.xml" "$RES_DIR/mipmap/ic_launcher.xml" "$RES_DIR/mipmap/ic_launcher_round.xml"; do
            if [ ! -f "$MIPMAP_TARGET" ]; then
              echo "$ADAPTIVE_ICON" > "$MIPMAP_TARGET"
            fi
          done
